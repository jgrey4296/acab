...
number.setParseAction       (lambda s, l, toks: int (toks[0]))
rest_of_line.setParseAction (lambda s, l, toks: toks[0])
save_kw.setParseAction      (lambda s, l, toks: RE.SAVE)
load_kw.setParseAction      (lambda s, l, toks: RE.LOAD)
multi_line.setParseAction   (build_multiline)
multi_line_pop.setParseAction(lambda s, l, toks: (RE.POP))
empty_line.setParseAction   (lambda s, l, toks: build_command(RE.NOP, params=[]))


break_cmd.setParseAction       (lambda s, l, toks: build_command(RE.BREAK))
state_io.setParseAction        (lambda s, l, toks: build_command(toks[0], params=toks[1:]))
base_statement.setParseAction  (lambda s, l, toks: build_command(RE.PASS, params=toks[:]))
nop_line.setParseAction        (lambda s, l, toks: build_command(RE.NOP, params=toks[:]))
decompose.setParseAction       (lambda s, l, toks: build_command(RE.DECOMPOSE, params=toks[:]))
listen_for.setParseAction      (lambda s, l, toks: build_command(RE.LISTEN, type="add", params=toks[:]))
listener_remove.setParseAction (lambda s, l, toks: build_command(RE.LISTEN, type="remove", params=toks[:]))
listeners.setParseAction       (lambda s, l, toks: build_command(RE.LISTEN, type="list", params=toks[:]))
listen_threshold.setParseAction(lambda s, l, toks: build_command(RE.LISTEN, type="threshold", params=toks[:]))
load_mod.setParseAction        (lambda s, l, toks: build_command(RE.MODULE, params=toks[:]))
manual_act.setParseAction      (lambda s, l, toks: build_command(RE.ACT, params=toks[:]))
print_state.setParseAction     (lambda s, l, toks: build_command(RE.PRINT, params=toks[:]))
reinit.setParseAction          (lambda s, l, toks: build_command(RE.INIT, params=toks[:]))
run_something.setParseAction   (lambda s, l, toks: build_command(RE.RUN, params=toks[:]))
step.setParseAction            (lambda s, l, toks: build_command(RE.STEP, params=toks[:]))
type_check.setParseAction      (lambda s, l, toks: build_command(RE.CHECK, params=toks[:]))
prompt_cmd.setParseAction      (lambda s, l, toks: build_command(RE.PROMPT, params=toks[:]))
exit_cmd.setParseAction        (lambda s, l, toks: build_command(RE.EXIT))
help_cmd.setParseAction        (lambda s, l, toks: build_command(RE.HELP, params=[]))
stats.setParseAction           (lambda s, l, toks: build_command(RE.STATS, params=toks[:]))
echo_kw.setParseAction         (lambda s, l, toks: build_command(RE.ECHO, params=[]))
