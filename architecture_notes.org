...
** wme
   The WME module defines a naive implementation of a WME based engine. Implements WMEs,
   Clauses, and the Engine.
...
** Norms in the engine
   The engine needs bayesian weighting of norm obedience, and underlying processes of internalization.
   Explicit linkings of norm deviance with 
