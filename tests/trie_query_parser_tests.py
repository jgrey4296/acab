...
class Trie_Query_Parser_Tests(unittest.TestCase):
    ...
    def test_basic_comparison(self):
        result = QP.comparison.parseString('>20, <40, !=$x, ==$y, ~=/blah/')[0]
        self.assertEqual(result[0], "constraints")
        self.assertEqual(len(result[1]), 5)
        self.assertTrue(all([isinstance(x, Comparison) for x in result[1]]))
        self.assertIsInstance(result[1][0].op, type(CompOp.op_list['>']))
        self.assertIsInstance(result[1][1].op, type(CompOp.op_list['<']))
        self.assertIsInstance(result[1][2].op, type(CompOp.op_list['!=']))
        self.assertIsInstance(result[1][3].op, type(CompOp.op_list['==']))
        self.assertIsInstance(result[1][4].op, type(CompOp.op_list['~=']))

    def test_basic_regex_comparison(self):
        result = QP.COMP_Internal.parseString('~= /blah/')[0]
        self.assertIsInstance(result, Comparison)
        self.assertIsInstance(result.op, type(CompOp.op_list['~=']))
    ...
    def test_basic_clause(self):
        ...
        self.assertEqual(result.components[-1]._data['op'], EXOP.DOT)
    ...
    def test_basic_clause_with_bind(self):
        ...
        self.assertEqual(result.components[-1]._value, 'c')
        self.assertEqual(result.components[-1]._data['op'], EXOP.DOT)
        self.assertTrue(result.components[-1]._data['bind'])
    ...
    def test_clause_fallback(self):
        ...
        self.assertEqual(result.fallback[0][0], 'x')
        self.assertEqual(result.fallback[0][1][-1]._value, 2)
    ...
    def test_clause_multi_fallback(self):
        ...
        self.assertEqual(result.fallback[0][0], 'x')
        self.assertEqual(result.fallback[0][1][-1]._value, 2)
        self.assertEqual(result.fallback[1][0], 'y')
        self.assertEqual(result.fallback[1][1][-1]._value, 5)
    ...
    def test_clause_fallback_strings(self):
        ...
        self.assertEqual(len(result.fallback), 2)
        self.assertEqual(result.fallback[0][0], 'x')
        self.assertEqual(result.fallback[0][1][-1]._value, 'c')
        self.assertEqual(result.fallback[1][0], 'y')
        self.assertEqual(result.fallback[1][1][-1]._value, 'e')
    ...
    #     # self.assertIsInstance(result.components[-1]
